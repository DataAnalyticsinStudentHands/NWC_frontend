...
function OralHistories() {
    ...
    useEffect(() => {
        fetch(`${import.meta.env.VITE_API_URL}/api/content-discover-stories?populate=*`)
        .then((res) => res.json())
        .then((data) => {
            ...
            profilepic: person.attributes.profilepic?.data  ? `${import.meta.env.VITE_API_URL}${person.attributes.profilepic.data.attributes.url}` : null,
    ...
    useEffect(() => {
        async function fetchAvailableStates() {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_API_URL}/api/content-discover-stories?fields=state&pagination[pageSize]=1000`
            );
    ...
    useEffect(() => {
		fetch([import.meta.env.VITE_API_URL, 'api/content-oral-history?populate=*'].join('/'))
			.then((res) => res.json())
			.then((data) => {
				const {
					data: {
