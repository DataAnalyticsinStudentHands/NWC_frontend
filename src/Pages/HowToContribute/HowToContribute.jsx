...
function HowToContribute() {
	...
	useEffect(() => {
		fetch(`${import.meta.env.VITE_API_URL}/api/content-how-to-contribute`)
			.then((res) => res.json())
			.then((data) => {
				const {
					data: {
        ...
        fetch(`${import.meta.env.VITE_API_URL}/api/content-how-to-contribute-resources?${resourceQuery}`)
            .then((res) => res.json())
            .then((data) => {
                let dataObj = {};
                data.data.forEach((item) => {
                    ...
                    ? imgUrl = `${import.meta.env.VITE_API_URL}${item.attributes.resource_icon.data.attributes.url}`
