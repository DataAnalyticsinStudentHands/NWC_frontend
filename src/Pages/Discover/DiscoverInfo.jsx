...
import InfoVideo from '../../Components/Avalon/InfoVideo.jsx';
...
function DiscoverInfo() {
    ...
    useEffect(() => {
        ...
        fetch(`${import.meta.env.VITE_API_URL}/api/content-discover-stories?filters[id][$eq]=${storyId}&populate=*`)
            .then(res => res.json())
            .then(data => {
                const {attributes:
                ...
                if (data.data[0].attributes.profilepic.data) {
                    profilepic = `${import.meta.env.VITE_API_URL}${data.data[0].attributes.profilepic.data.attributes.url}`;
