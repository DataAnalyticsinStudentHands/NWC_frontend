...
function AdvancedSearch() {
  ...
  useEffect(() => {
    async function fetchContentMap() {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/content-mapping-nwc`);
        if (!response.ok) {
  ...
  async function fetchAvailableStates() {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/nwc-participants?fields=represented_state&pagination[pageSize]=1000`
      );
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/api/data-careers?sort=category_of_employment`)
      .then(res => res.json())
      .then(data => {
    ...
    useEffect(() => {
      fetch(`${import.meta.env.VITE_API_URL}/api/data-leadership-in-organizations?sort=role`)
        .then(res => res.json())
        .then(data => {
    ...
    useEffect(() => {
      fetch(`${import.meta.env.VITE_API_URL}/api/data-planks?sort=plank`)
        .then(res => res.json())
        .then(data => {
  ...
  async function onSubmit(data) {
    ...
    if (array_query.length === 0) {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/nwc-participants?${query}`).then(res => res.json());
