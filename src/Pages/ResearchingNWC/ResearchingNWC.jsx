...
function ResearchingNWC() {
  ...
  useEffect(() => {
    async function fetchContentMap() {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/content-mapping-nwc?populate=*`);
        if (!response.ok) {
        ...
        <img
          src={
            contentMap?.attributes?.BasicSearch_Progress_Map.data
              ? [import.meta.env.VITE_API_URL, contentMap.attributes.BasicSearch_Progress_Map.data.attributes.url].join('')
              : progress_map
          }
          alt="Progress Map"
