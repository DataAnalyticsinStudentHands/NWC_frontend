...
function MeetTheTeam() {
  ...
  const fetchData = async () => {
    ...
    fetch([import.meta.env.VITE_API_URL, 'api/content-about-project-leads?_sort=Order:ASC&populate=*'].join('/'))
    .then(res => res.json())
    .then(data => {
        setLeads(data.data)
    })
  ...
  const res = await fetch([import.meta.env.VITE_API_URL, 'api/content-about-collaborators?_sort=LastName:ASC&_limit=-1&populate=*'].join('/'))
  ...
  return <div className="meet">
    ...
    <div className="meetLeads">
      {leads.map((value) => <MeetIcon
        img={import.meta.env.VITE_API_URL + value.attributes.Illustration.data.attributes.url}
        imghover={import.meta.env.VITE_API_URL + value.attributes.Illustration_hover.data.attributes.url}
        name={value.attributes.Name}
        role={value.attributes.Role}
        pfp={import.meta.env.VITE_API_URL + value.attributes.ProfilePicture.data.attributes.url}
              ...
              {currentTab !== "STEERING COMMITTEE" && currentTab !== "INAUGURAL TEAM" ?
                ...
                <div className="aboutTable_steering_entry">
                  {content.attributes.ProfilePicture.data ?
                    <img src={import.meta.env.VITE_API_URL + content.attributes.ProfilePicture.data.attributes.url} alt="_" /> :
