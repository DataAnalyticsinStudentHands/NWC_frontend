...
function Home() {
  ...
  const overlaymp4 = import.meta.env.VITE_API_URL + "/uploads/overlayvid_compressed_381cfa05b6.mp4";
  ...
  useEffect(() => {
    fetch([import.meta.env.VITE_API_URL, 'api/content-home'].join('/'))
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch([import.meta.env.VITE_API_URL, 'api/content-home-maps?populate=*'].join('/'))
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch([import.meta.env.VITE_API_URL, "api/content-home-highlights?populate=*"].join('/'))
    .then(res => res.json())
    .then(data => {
        ...
        setImages(
            data.data.map(d => {
                ...
                const thumbnail = [import.meta.env.VITE_API_URL, d.attributes.Thumbnail.data.attributes.url].join('')
                const id = d.id;
